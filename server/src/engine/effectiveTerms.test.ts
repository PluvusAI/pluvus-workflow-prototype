/**
 * Unit tests for the PLU-137/138 effective-terms resolver (snapshot-wins).
 * Pure logic — run with:
...
// 11. B2 rescue: a config with a floor but NO ceiling, pinned to a policy
//     snapshot that HAS a ceiling → the effective band gains the ceiling, so the
//     H1 no-ceiling escalation must NOT trip. This is why H1 defers to the
...
// 12. 0/0 commission-only: recommendedOfferPosition / overCeilingTolerance are
...
// 13. present-but-null snapshot value CLEARS a stale config value (does NOT retain
