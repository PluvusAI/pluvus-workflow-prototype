/**
 * PLU-137/138 — snapshot integrity guard.
 ...
 *      error) — and does so BEFORE any DB read (proven by a client that
 *      throws on use).
...
test("escalateSnapshotIntegrity is terminal MANUAL_REVIEW with NO derived-fact writes", () => {
  // A late guard placement invoking the agent would persist an agent-derived
  // memory write alongside MANUAL_REVIEW. Moving it before the agent is only
  // safe because the result is pure and carries no agent-derived writes —
  // assert that property directly.
