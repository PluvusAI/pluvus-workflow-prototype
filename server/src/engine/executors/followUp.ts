...
export async function executeFollowUp(
  ...
  // PLU-137/138: deliverables/timeline/rewardDescription are no longer stamped
  // into node config (restampBrand stopped copying material terms), and post-
  // PLU-135 they live on CampaignDetails, not the Campaign row — so a bare
  // ctx.campaign fallback resolves them to undefined and the follow-up copy
  // can hallucinate them. Build the fallback from CampaignDetails via
  // toCampaignBrandFields, matching the operatorHandoff/brandApproval executors.
