...
export interface ContextRecord {
  ...
  // PLU-138 §obs — the private BAND fell back to nodeGraph because no policy snapshot
  // was pinned. DISTINCT from legacyFallbackUsed (which tracks the PUBLIC terms
  // snapshot): a gift/affiliate journey can have a terms snapshot but a null fee band.
  bandLegacyFallback: boolean;
  // PLU-138 §obs — the effective source of the material terms used this turn:
  // 'snapshot' when either snapshot supplied a value, 'legacy_nodegraph' when both
  // the band and the public terms fell back to config. Lets an operator trace which
  // authority a runtime decision used.
