/**
 * PLU-137/138 integration: prove the snapshot-wins resolver flows THROUGH the
...
test("no-snapshot journey → request identical to raw-config request", () => {
  ...
  const { config: effectiveConfig, source, bandLegacyFallback } = resolveEffectiveNegotiationConfig({
    config,
  });
