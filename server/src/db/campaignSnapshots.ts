...
// PLU-137/138: the post-acceptance executors (contentBrief / rewardSetup /
...
// equivalent of loadPinnedSnapshots (negotiation.ts) for the PUBLIC terms only: it
// reads the pinned CampaignTermsSnapshot by id with the SAME integrity discipline
// (missing / cross-campaign / campaign-unresolved are explicit failures, never a
// silent config read), so a stale or mis-pinned snapshot escalates instead of
// quietly falling back to nodeGraph.
//
// Returns { terms } on success (or when nothing is pinned → {}, the legacy
// no-snapshot journey — the caller keeps its existing config chain), or
...
export async function loadPinnedTermsSnapshotForExecutor(
  ...
  // Fail CLOSED (same discipline as loadPinnedSnapshots's campaign_unresolved
  // guard): a pin exists but the campaign could not be resolved — never fall
  // through to a silent no-verification read.
  if (!expectedCampaignId) return { integrityFailure: { reason: "campaign_unresolved" } };
  ...
  if (terms.campaignId !== expectedCampaignId) {
