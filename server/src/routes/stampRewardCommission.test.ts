/**
 * Unit tests for stampRewardFromNegotiation. PLU-137/138 REVERSED its behavior:
...
// ── PLU-137/138: restampBrand no longer stamps MATERIAL terms ─────────────────
...
test("restampBrand STILL stamps brand presentation + shipsPhysicalProduct (retained)", () => {
  ...
  assert.equal(cfg["shipsPhysicalProduct"], true); // compat flag retained (PLU-183/184 owns removal)
});
